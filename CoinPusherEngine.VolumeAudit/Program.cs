...
var cases = Enumerable.Range(0, count)
    .Select(i => (Index: i, Seed: rng.Next(1, int.MaxValue), Prizes: BuildInput(i, rng, inputMode).ToArray()))
    .ToArray();
inputStage.Stop();
...
static IReadOnlyList<decimal> BuildInput(int index, Random rng, string inputMode) =>
    inputMode switch
    {
