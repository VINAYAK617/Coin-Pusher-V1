...
internal static class K
{
    ...
    //   - CapacityAnalyzer.MinExtraSpins(...) < 0 (infeasible without it) forces
    ...
    //     only ever added when CapacityAnalyzer proves it's required, never "for fun."
    ...
    internal static double Weight(string envName, double fallback)
    {
        var raw = Environment.GetEnvironmentVariable(envName);
        if (string.IsNullOrWhiteSpace(raw)) return fallback;
        return double.TryParse(raw, out var value)
