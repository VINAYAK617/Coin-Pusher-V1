...
public sealed class CoinPusherTicketGenerator
{
    ...
    public CoinPusherTicketGenerationResult Generate(
        ...
        int? seed = null) =>
        GenerateCore(prizeAmounts, exactPpsCombinationId: null, seed);

    public CoinPusherTicketGenerationResult GeneratePpsCombination(
        int ppsCombinationId,
        int? seed = null)
    {
        var combination = _settings.PpsCombinations
            .SingleOrDefault(item => item.Id == ppsCombinationId);
        if (combination == null)
        {
            return new CoinPusherTicketGenerationResult(
                CoinPusherTicketGenerationStatus.InvalidRequest,
                $"PPS combination #{ppsCombinationId} is not configured",
                seed ?? 0,
                null,
                null,
                null,
        ...
        return GenerateCore(
            new[] { combination.TotalPrize },
            ppsCombinationId,
            seed);
    }

    private CoinPusherTicketGenerationResult GenerateCore(
        IReadOnlyList<decimal>? prizeAmounts,
        int? exactPpsCombinationId,
        int? seed)
    {
        ...
        for (var attempt = 0; attempt < maxAttempts; attempt++)
        {
            ...
            var generated = new ForwardTicketGenerator(_settings, attemptSeed).Generate(
