...
public sealed class CoinPusherTicketJsonGenerator
{
    ...
    public CoinPusherTicketJsonGenerationResult Generate(
        ...
        int? seed = null)
    {
        return FromGuard(_guard.Generate(prizeAmounts, seed));
    }

    public CoinPusherTicketJsonGenerationResult GeneratePpsCombination(
        int ppsCombinationId,
        int? seed = null)
    {
        return FromGuard(_guard.GeneratePpsCombination(ppsCombinationId, seed));
    }

    private static CoinPusherTicketJsonGenerationResult FromGuard(
        CoinPusherTicketGenerationGuardResult guard)
    {
