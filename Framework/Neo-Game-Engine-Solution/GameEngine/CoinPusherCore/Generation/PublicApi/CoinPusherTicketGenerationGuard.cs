...
public sealed class CoinPusherTicketGenerationGuard
{
    ...
    internal CoinPusherTicketGenerationGuard(ICustomProfileSettings settings)
    {
        _settings = settings;
    ...
    public CoinPusherTicketGenerationGuardResult Generate(
        ...
        int? seed = null) =>
        Validate(_generator.Generate(prizeAmounts, seed));

    public CoinPusherTicketGenerationGuardResult GeneratePpsCombination(
        int ppsCombinationId,
        int? seed = null) =>
        Validate(_generator.GeneratePpsCombination(ppsCombinationId, seed));

    private CoinPusherTicketGenerationGuardResult Validate(
        CoinPusherTicketGenerationResult generation)
    {
